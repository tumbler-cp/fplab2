...
module PreDict =
    ...
    let insert (key: string) (value: 'T) (trie: Trie<'T>) : Trie<'T> =
        let rec insertHelper (node: TrieNode<'T>) (key: string) (index: int) : TrieNode<'T> =
            ...
            | true -> { node with Value = Some value; IsTerminal = true }
            | false ->
                let char = key.[index]
                let childNode =
                    match Map.tryFind char node.Children with
                    | Some n -> n
                    | None -> { Value = None; Children = Map.empty; IsTerminal = false }
                let updatedChild = insertHelper childNode key (index + 1)
                { node with Children = Map.add char updatedChild node.Children }
        { trie with Root = insertHelper trie.Root key 0 }
    ...
    let remove (key: string) (trie: Trie<'T>) : Trie<'T> =
        let rec removeHelper (node: TrieNode<'T>) (key: string) (index: int) : TrieNode<'T> option =
            ...
            | true ->
                ...
                | true ->
                    let updatedNode = { node with Value = None; IsTerminal = false }
                    if updatedNode.Children.IsEmpty then None else Some updatedNode
                | false -> Some node
            | false ->
                let char = key.[index]
                match Map.tryFind char node.Children with
                | Some childNode ->
                    ...
                    | Some updatedChild ->
                        Some { node with Children = Map.add char updatedChild node.Children }
                    | None ->
        ...
        | None -> { trie with Root = { Value = None; Children = Map.empty; IsTerminal = false } }
        ...
        let rec filterHelper (node: TrieNode<'T>) : TrieNode<'T> option =
            let filteredChildren =
                ...
                |> Seq.choose (fun (char, childNode) ->
                    match filterHelper childNode with
                    | Some filteredChild -> Some (char, filteredChild)
                    | None -> None)
                |> Map.ofSeq
            let isTerminal = node.IsTerminal && Option.exists predicate node.Value
            if isTerminal || not filteredChildren.IsEmpty then
                Some { node with Children = filteredChildren; IsTerminal = isTerminal }
            else None
    ...
    let map (transform: 'T -> 'U) (trie: Trie<'T>) : Trie<'U> =
        let rec mapHelper (node: TrieNode<'T>) : TrieNode<'U> =
            ...
            let mappedChildren =
                node.Children
                |> Map.map (fun _ childNode -> mapHelper childNode)
            { Value = mappedValue; Children = mappedChildren; IsTerminal = node.IsTerminal }
    ...
    let empty : Trie<'T> = 
        { Root = { Value = None; Children = Map.empty; IsTerminal = false } }
        ...
        let rec mergeNodes (node1: TrieNode<'T>) (node2: TrieNode<'T>) : TrieNode<'T> =
            ...
            let mergedChildren =
                ...
                |> Map.fold (fun acc key childNode1 ->
                    ...
                    | None -> Map.add key childNode1 acc) node2.Children
            { Value = mergedValue; Children = mergedChildren; IsTerminal = node1.IsTerminal || node2.IsTerminal }
    ...
    let compareTrees (tree1: Trie<'T>) (tree2: Trie<'T>) : bool =
        compareNodes tree1.Root tree2.Root
